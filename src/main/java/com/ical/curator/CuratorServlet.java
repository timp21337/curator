...
//import com.ical.curator.poem.CuratorDatabase;

/**
 * Base servlet for Curator servlets.
 */
public abstract class CuratorServlet extends TemplateServlet {
  ...
  private boolean fileAt(String filename){
    ...
    if (filename.equals("/")) return false;
    String fsName = "/dist/curator/www" + filename;
    File it = new File(fsName);
    System.err.println("FS:" + fsName + " " + it.exists());
    return it.exists();
  }

  public String curatorTemplate(String name) {
  ...
  protected String doTemplateRequest(Melati melati, ServletTemplateContext context)
      throws Exception {
    return curatorTemplate(reallyDoTemplateRequest(melati, context));
  }
